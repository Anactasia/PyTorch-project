...
class RandomBlur:
    ...
    def __call__(self, img):
        ...
        return transforms.ToTensor()(img.filter(ImageFilter.GaussianBlur(radius)))
...
class RandomPerspective:
    ...
    def __call__(self, img):
        ...
        if random.random() > self.p:
            return transforms.ToTensor()(img)
        aug = transforms.RandomPerspective(distortion_scale=self.distortion_scale, p=1.0)
        return transforms.ToTensor()(aug(img))
