...
def test_torch_model(
    ...
    **kwargs
    ...
    model.eval()
    device = 'cuda' if torch.cuda.is_available() and timer_type == 'cuda' else 'cpu'
...
def benchmark_models(
    ...
    real_dataset = CustomImageDataset(root_dir='core/ДЗ_7/data/test', target_size=image_size)
    dummy_dataset = RandomImageDataset(target_size=(3, image_size[0], image_size[1]))
...
if __name__ == '__main__':
    for size in [224, 256, 384, 512]:
        print(f"\n===== Running benchmark for size {size}x{size} =====")
        benchmark_models(
